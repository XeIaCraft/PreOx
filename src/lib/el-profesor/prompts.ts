...
Ta tâche : rédige UN VRAI CHAPITRE de synthèse sur cette notion — pas un empilement de fiches, un texte structuré et agréable à lire comme un chapitre de livre de référence, organisé en SECTIONS TITRÉES PAR SUJET. Respecte ces règles strictes, dans cet ordre de priorité :

1. RÈGLE LA PLUS IMPORTANTE — ORGANISE PAR SUJET, JAMAIS PAR TYPE D'INFORMATION : chaque source précise entre parenthèses la fiche dont elle vient (ex. « — Suxaméthonium — ») — c'est ton indice du sujet réel. Une notion regroupe souvent PLUSIEURS sujets distincts (plusieurs médicaments d'une même classe, plusieurs appareils, plusieurs techniques...) traités séparément par les livres sources. Identifie ces sujets distincts et crée UNE section par sujet (fusionne seulement les sujets qui sont en réalité LE MÊME élément traité par des livres différents, ex. deux fiches "Suxaméthonium" venant de deux livres). À l'intérieur de la section d'un sujet donné, enchaîne naturellement définition, mécanisme, valeurs, usage pratique et pièges pour CE sujet précis. Ce qui est STRICTEMENT INTERDIT : créer une section "Définitions" qui empile les définitions de plusieurs sujets différents, puis une section "Valeurs" qui empile leurs valeurs, etc. — ce format en est totalement illisible et inutilisable pédagogiquement, l'ordre de lecture doit toujours rester "un sujet à la fois", jamais "un type d'info à la fois" à travers plusieurs sujets. Titre chaque section du nom du sujet lui-même (ex. "Suxaméthonium", "Rocuronium"), jamais d'un type d'information (jamais "Définitions", "Valeurs", "Pièges" comme titre de section).
2. Regroupe les sections de sujets à fonction ou famille similaire les unes à côté des autres (ex. tous les curares dépolarisants ensemble, puis tous les curares non dépolarisants) plutôt que dans un ordre arbitraire ou calqué sur l'ordre des livres sources. Si plusieurs sujets partagent un mécanisme ou une base commune (ex. la physiologie normale avant les curares eux-mêmes), place cette base commune dans sa propre section, en premier, avant les sections spécifiques à chaque sujet.
3. Dans la section d'un même sujet, si plusieurs blocs sources disent la même chose (même en termes différents, même venant de livres différents), fusionne-les en un seul bloc de synthèse — ne répète jamais un fait déjà couvert par un bloc de synthèse précédent, dans cette section ou une autre.
4. Si un bloc source apporte un détail complémentaire propre à un livre (absent des autres) sur ce même sujet, garde-le — soit comme nuance dans un bloc existant, soit comme bloc à part dans la même section si le sous-thème diffère assez — ne perds jamais une information réellement utile sous prétexte de dédupliquer.
5. Types de blocs disponibles, mêmes règles de contenu qu'une extraction normale :
${BLOCK_TYPES_DOC}
Ne fusionne jamais deux blocs sources de nature différente (ex. un tableau comparatif et une définition) dans un seul bloc de synthèse — ce sont deux blocs distincts, même s'ils traitent du même sujet.
6. Si deux blocs sources sur le même sujet se contredisent factuellement sur un point précis (valeur numérique différente, conduite à tenir opposée), ne tranche PAS toi-même lequel a raison : garde les deux formulations dans le même bloc de synthèse en signalant explicitement la divergence (« Selon [nom du livre A] ... alors que selon [nom du livre B] ... »).
7. EXHAUSTIVITÉ — AUCUNE PERTE D'INFORMATION : chaque bloc source listé ci-dessus doit être repris par au moins un bloc de synthèse, quelque part dans la section de son sujet. Avant de répondre, repasse mentalement la liste complète des identifiants [b1], [b2], [b3]... et vérifie qu'aucun n'est absent de tous les "source_block_ids" de ta réponse — un bloc source qui n'apporte vraiment rien d'utile (redondance totale déjà couverte ailleurs) doit quand même apparaître dans le "source_block_ids" du bloc de synthèse qui couvre ce même fait, jamais être simplement omis.
