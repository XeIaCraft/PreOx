...
function readValues(text: string): { values: QuickValue[]; asa?: number; mallampati?: 1 | 2 | 3 | 4; sex?: "M" | "F" } {
  ...
  const sex = /\b(madame|mme|patiente|nee le|femme|fillette|fille\s+(de\s+)?\d+|sexe\s*:?\s*f)\b/.test(f) ? "F" : /\b(monsieur|mr|m\. [a-z]|ne le|homme|garcon|sexe\s*:?\s*m)\b/.test(f) ? "M" : undefined;
